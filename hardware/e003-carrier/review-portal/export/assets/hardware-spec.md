...
E003. It complements the product-level [Hardware v2 specification](https://github.com/zentala/desk.zentala.io/blob/dev/research/hardware-v2-spec.md)
...
| Blue `VL53LDK`-marked breakout | four electrical pads `VIN`, `GND`, `SCL`, `SDA` on one edge; two auxiliary plated vias marked `X` and `e` on the opposite edge; optical sensor package and corner hole | exact silicon identity, regulator/level shifting, pull-ups, outline, optical keep-out and pitch |
...
The breakout's two opposite-edge features marked `X` and `e` are visible in the
received photo and are treated as auxiliary plated vias, not module pins. They
are shown in the carrier review render only as reference marks and are not
included in the carrier schematic or routing.
...
| GY-530-style `VL53LDK` board | 10.5 × 13.3 mm; one 3 mm corner hole; four labelled I²C/power pads on the lower edge and two auxiliary plated vias on the opposite edge | **Strong visual match**: same blue rectangle, corner hole, optical package and VIN/GND/SCL/SDA order | Keep as provisional mechanical envelope; auxiliary vias are reference geometry only and are not carrier nets |
